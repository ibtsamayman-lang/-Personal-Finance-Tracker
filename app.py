...
# app.py
# -------
# Personal Finance Tracker - Streamlit UI.
...
# # تنسيق شكل الصفحة (CSS مخصص) - مفيهوش أي منطق، شكل بس
# st.markdown("""
# <style>
#     .stApp {
...
#     /* حقول الإدخال العادية (نص، رقم، تاريخ) */
...
#     /* تغطية شاملة لأي قائمة منسدلة تظهر فوق الصفحة (Portal) */
...
# st.caption("Track your income, expenses, and stay within budget.")

# # آخر عملية اتسجلت (تاريخ ووقت)
# last_info = manager.last_transaction_info()
# if last_info is not None:
#     st.info(f"🕒 Last transaction recorded on: {last_info['created_at']}")
# else:
#     st.info("🕒 No transactions yet. Start from the 'Add Transaction' tab!")
...
#     c3.metric("🏦 Balance", f"{balance:,.2f}", delta=None)
...
#         if by_cat:
#             fig = px.pie(names=list(by_cat.keys()), values=list(by_cat.values()), hole=0.5)
...
#         if not trend.empty:
#             fig = px.bar(trend, x="month", y="signed", labels={"signed": "Net (Income - Expenses)"})
...
#         st.caption("No budgets set. Go to the Budgets tab to set spending limits per category.")
#     else:
#         for b in budget_status:
#             icon = CATEGORY_ICONS.get(Category(b["category"]), "📦")
#             if b["exceeded"]:
#                 st.error(f"{icon} **{b['category']}**: spent {b['spent']:,.2f} of {b['limit']:,.2f} — over budget!")
#             elif b["percent"] >= 80:
#                 st.warning(f"{icon} **{b['category']}**: spent {b['spent']:,.2f} of {b['limit']:,.2f} ({b['percent']:.0f}%)")
#             else:
#                 st.success(f"{icon} **{b['category']}**: spent {b['spent']:,.2f} of {b['limit']:,.2f} ({b['percent']:.0f}%)")
...
#     if not comparison:
#         st.info("Not enough data yet to compare months.")
#     else:
#         for c in comparison:
...
#             st.write(
#                 f"{arrow} **{c['category']}**: {c['current']:,.2f} this month "
#                 f"(was {c['previous']:,.2f} last month, change {c['change_percent']:+.1f}%)"
#             )
...
#         c1, c2 = st.columns(2)
#         t_type = c1.selectbox("Type", [t.value for t in TransactionType])
#         t_category = c2.selectbox("Category", [c.value for c in Category])
#         c3, c4 = st.columns(2)
#         t_amount = c3.number_input("Amount", min_value=0.0, step=1.0)
...
#             else:
#                 manager.add_transaction(t_type, t_category, t_amount, t_note, t_date.strftime("%Y-%m-%d"))
...
#     if not transactions:
#         st.info("No transactions yet. Add one from the 'Add Transaction' tab.")
#     else:
#         filter_type = st.selectbox("Filter by type", ["All"] + [t.value for t in TransactionType])
#         filtered = transactions if filter_type == "All" else [t for t in transactions if t.type == filter_type]

#         for t in filtered:
#             c1, c2, c3, c4, c5 = st.columns([1, 2, 2, 3, 1])
#             sign = "+" if t.type == TransactionType.INCOME.value else "-"
#             color = "green" if t.type == TransactionType.INCOME.value else "red"
#             c1.markdown(f"{t.icon()}")
#             c2.markdown(f"**{t.category}**")
#             c3.markdown(f":{color}[{sign}{t.amount:,.2f}]")
#             c4.markdown(f"{t.date} — {t.note or ''}")
...
#     st.caption("Set a monthly spending limit per category. You'll get alerts on the Dashboard when you get close or go over.")

#     with st.form("budget_form"):
#         b_category = st.selectbox("Category", [c.value for c in Category if c != Category.SALARY])
#         b_limit = st.number_input("Monthly limit", min_value=0.0, step=10.0)
...
#             st.markdown(f"- {CATEGORY_ICONS.get(Category(cat), '📦')} **{cat}**: {limit:,.2f} / month")
